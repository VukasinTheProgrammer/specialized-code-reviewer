...
**Two separate questions, not one.** "Is this citation real" and "does this belong under this label" are different judgment calls, and a reviewer moving fast tends to only ask the first. A well-specified proposer prompt already polices citation quality reasonably well on its own — a controlled comparison (`todays-work/week9/wednesday.md`) found a full-auto run's citations as clean as, and on two patterns cleaner than, a human-reviewed one. What that same comparison caught, and what the citation check alone never would have: a record whose `label` was one of the closed 15 (syntactically valid, passed `validate-pack.sh` clean) but semantically wrong for what this project actually uses that label to mean — a local-file atomic-write pattern filed under `db`, which in every other pack in this repo means real database access (SQL, a store with query semantics), not file I/O. Nothing about that record's citations was broken; the mistake was invisible to a citation-only read.

So for every candidate, ask explicitly: **does this match how `label` is actually used elsewhere in this pack (or the repo's other packs, if this is a first record under that label)?** Skim a sibling record under the same label if one exists — same pack or another partner pack — before accepting; a candidate that would read strangely next to that sibling is a labeling problem even when its citations are perfect. This is the check a citation-verification pass structurally cannot do, and it is the actual reason a human belongs in this loop — treat it as load-bearing, not a nice-to-have on top of citation review.
